import { system, world, Block, Dimension, ItemStack } from "@minecraft/server";
...
world.beforeEvents.worldInitialize.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent(
        "gbaported:custom_door",
        CustomDoorBlockComponent
    );
});
...
function doorUpdate(block : Block, dir, state, set) {
...
function doorPlaced(data){
    if(data.block.above(1).matches("minecraft:air")) {
        ...
        doorTop.setPermutation(doorTop.permutation.withState("minecraft:cardinal_direction", 
            data.block.permutation.getState("minecraft:cardinal_direction")));
    } else {
