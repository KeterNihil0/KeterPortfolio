...
function addSpeedToPlayer() {
    ...
    players.forEach(p => {
        ...
        if (((pv2.y > 0)) && (Math.abs(p.getVelocity().x) > 0 || (Math.abs(p.getVelocity().z) > 0))) {
            acc_z = 1;
        }
        else {
            acc_z = -1;
        ...
        if (+p.getProperty("cpm:acc_z") > 10) {
            p.addEffect('speed', 15, { amplifier: 1, showParticles: false });
            p.addEffect('jump_boost', 15, { amplifier: 1, showParticles: false });
        }
        if (+p.getProperty("cpm:acc_z") > 30) {
            p.addEffect('speed', 15, { amplifier: 3, showParticles: false });
            p.addEffect('jump_boost', 15, { amplifier: 2, showParticles: false });
        }
        if (+p.getProperty("cpm:acc_z") > 60) {
            p.addEffect('speed', 15, { amplifier: 4, showParticles: false });
            p.addEffect('jump_boost', 15, { amplifier: 3, showParticles: false });
        }
...
w.beforeEvents.worldInitialize.subscribe(({ blockComponentRegistry }) => {
    blockComponentRegistry.registerCustomComponent("cpm:swater_tick", SWaterTick);
});
