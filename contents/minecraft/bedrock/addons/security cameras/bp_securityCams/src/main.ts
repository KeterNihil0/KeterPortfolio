import { Block, Dimension, Entity, EntityComponentTypes, EntityInventoryComponent, ItemStack, PaletteColor, Player, system as s, Vector3, world as w } from "@minecraft/server";
...
function createCamera(player : Player, monitor : ItemStack, block : Block) {
    ...
    let playerContainer = player.getComponent(EntityComponentTypes.Inventory) as EntityInventoryComponent;
    let newMonitor = monitor.getDynamicProperty(newCam) as Vector3;
...
function openCamForm(player : Player, monitor : ItemStack) {
    ...
    for (let i = 0; i < monitorCams.length; i++) {
        { 
            ...
            let cam = getTileEntity(player.dimension.getBlock(monitor.getDynamicProperty(monitorCam) as Vector3));
            if (cam != undefined) {
    ...
    form.show(player).then(r => {
        if (monitorCamButtons.length > 0) {
            ...
            if (response != undefined && response != monitorCamButtons.length) {
                let cam = getTileEntity(player.dimension.getBlock(monitor.getDynamicProperty(monitorCams[response]) as Vector3))
                if (cam != undefined) {
...
function openCamButtonForm(player : Player, monitor : ItemStack, camera) {
    let form = new ActionFormData();
...
function openCamAdjustForm(player : Player, monitor : ItemStack, camera) {
    let form = new ModalFormData();

    
    form.title("Camera Adjust");
    let camName = form.textField("Camera Name: ", camera.nameTag, camera.nameTag)
    let yaw = form.slider("Yaw Angle: ", -90, 90, 5, camera.getProperty("sec_cams:yaw") as number);
    let pitch = form.slider("Pitch Angle: ", -45, 45, 5, camera.getProperty("sec_cams:pitch") as number);
    ...
    form.show(player).then(r => {
        ...
        camera.nameTag = camName as string;
...
function checkIfCamPresent(player : Player, cam) {
    ...
    player.runCommandAsync(`/camera @s set minecraft:free rot ${(cam.getProperty("sec_cams:pitch") as number)} ${(cam.getProperty("sec_cams:yaw") as number) + (cam.getProperty("sec_cams:dir") as number) }`);
}

function updateCams(player : Player, monitor : ItemStack) {
    ...
    for (let i = 0; i < camIdsOld.length; i++) {
        { 
            let camLoc = (monitor.getDynamicProperty(camIdsOld[i]) as Vector3)
...
function spawnTileEntity(block : Block) {
...
function getTileEntity(block : Block) {
    if (block == undefined) {
        return;
    }
...
function validateTileEntity(entity : Entity) {
