import { 
    ...
    Vector3,
    Block,
    EntityComponentTypes as EntityCT,
    ItemComponentTypes as ItemCT,
    ItemStack,
    DimensionTypes,
    Entity,
    BlockStateType,
...
w.beforeEvents.playerInteractWithBlock.subscribe((data) => {
    if(data.itemStack.typeId == "km:debug_tool" && data.isFirstEvent) {s.run(()=>{debugTool(data)})}
})

function machinePlaced(block: Block) {
...
function wirePlaced(block: Block) {
...
function assignwireLayout(dim: string) {
    for (const e of w.getDimension(dim).getEntities()) {
        if (e.getComponent(EntityCT.TypeFamily)?.hasTypeFamily("km_wire")) {
            ...
            if (e_block?.isValid() !== true) {return;}
            
            let loc1 = e_block?.location;
            if (loc1 === undefined) {return;}
            ...
            for (const n of e.dimension.getEntities({maxDistance:1, location:e.location})) {
                if (n.getComponent(EntityCT.TypeFamily)?.hasTypeFamily("km_conductor")) {
                    ...
                    if (n_block.isValid() !== true) {return;}
                    ...
                    if((loc1.x == loc2.x+1) && n_block?.hasTag("km_conducts_e")) {e.setProperty("km:w",true); c++;}
                    if((loc1.x == loc2.x-1) && n_block?.hasTag("km_conducts_w")) {e.setProperty("km:e",true); c++;}
                    if((loc1.y == loc2.y+1) && n_block?.hasTag("km_conducts_d")) {e.setProperty("km:u",true); c++;}
                    if((loc1.y == loc2.y-1) && n_block?.hasTag("km_conducts_u")) {e.setProperty("km:d",true); c++;}
                    if((loc1.z == loc2.z+1) && n_block?.hasTag("km_conducts_s")) {e.setProperty("km:n",true); c++;}
                    if((loc1.z == loc2.z-1) && n_block?.hasTag("km_conducts_n")) {e.setProperty("km:s",true); c++;}
                }
            }
        }
    }
...
function floodNetwork(dim : Dimension, coords : Vector3, originator : Entity, netId : string) {
    ...
    if (getTileEntity(dim, coords).getDynamicProperty("km:flooded") == true) {return;}
    ...
    if(dim.getBlock({x:x+1,y:y,z:z}).hasTag("km_conducts_w")) { floodNetwork(dim, {x:x+1,y:y,z:z}, originator, newNetId); }
    if(dim.getBlock({x:x-1,y:y,z:z}).hasTag("km_conducts_e")) { floodNetwork(dim, {x:x-1,y:y,z:z}, originator, newNetId); }
    if(dim.getBlock({x:x,y:y,z:z+1}).hasTag("km_conducts_n")) { floodNetwork(dim, {x:x,y:y,z:z+1}, originator, newNetId); }
    if(dim.getBlock({x:x,y:y,z:z-1}).hasTag("km_conducts_s")) { floodNetwork(dim, {x:x,y:y,z:z-1}, originator, newNetId); }
    if(dim.getBlock({x:x,y:y+1,z:z}).hasTag("km_conducts_u")) { floodNetwork(dim, {x:x,y:y+1,z:z}, originator, newNetId); }
    if(dim.getBlock({x:x,y:y-1,z:z}).hasTag("km_conducts_d")) { floodNetwork(dim, {x:x,y:y-1,z:z}, originator, newNetId); }
}

function resetFlood(dim : Dimension, full : boolean) {
...
function netUpdate(dim : Dimension) {
    resetFlood(dim, true);
    for (const e of dim.getEntities()) {
        if (e.getComponent(EntityCT.TypeFamily)) {
            if (e.getComponent(EntityCT.TypeFamily).hasTypeFamily("km_turbine")) {
                if (e.getDynamicProperty("km:flooded") == false || e.getDynamicProperty("km:flooded") === undefined) {
                    let block = e.dimension.getBlock(e.location);
                    floodNetwork(dim, block.location, e, e.getDynamicProperty("km:original_netId") as string)
                }
            }
        }
    }
...
function getNetworkEu(entity : Entity) {
    let netId = entity.getDynamicProperty("km:netId");

}

class handleNetworkEu {
    static request(entity : Entity, amount) {
    ...
    static offer(entity : Entity, amount) {
        let netId = "km:network_"+String(entity.getDynamicProperty("km:netId"));
        let networkEu = Number(w.getDynamicProperty(netId));
...
function fluidFlow(dim : Dimension) {
    for (const e of dim.getEntities()) {
                ...
                for (const n of dim.getEntities({maxDistance : 1, location : e.location})) {
                    if (n.getComponent(EntityCT.TypeFamily)?.hasTypeFamily("km_fluid_conductor") && (n != e)) {
                        if (n.getDynamicProperty("km:fluid_stored") == "none" || n.getDynamicProperty("km:fluid_stored") == undefined) {
                            ...
                            if ((e.location.y+1==n.location.y) && (ef.hasTypeFamily("km_pipe_u")) && (nf.hasTypeFamily("km_pipe_d"))) {potentialTargets.push({e:n, y:1})}
                            if ((e.location.y-1==n.location.y) && (ef.hasTypeFamily("km_pipe_d")) && (nf.hasTypeFamily("km_pipe_u"))) {potentialTargets.push({e:n, y:-1})}
                            if ((e.location.z+1==n.location.z) && (ef.hasTypeFamily("km_pipe_n")) && (nf.hasTypeFamily("km_pipe_s"))) {potentialTargets.push({e:n, y:0})}
                            if ((e.location.z-1==n.location.z) && (ef.hasTypeFamily("km_pipe_s")) && (nf.hasTypeFamily("km_pipe_n"))) {potentialTargets.push({e:n, y:0})}
                        }
                    }
                }
...
function fluidPass(isPipe : boolean, o : Entity, t : Entity) {
        ...
        for (const e of w.getDimension(dim).getEntities()) {
            if (e.getComponent(EntityCT.TypeFamily)?.hasTypeFamily("km_machine")) {
                ...
                if ((block?.isValid() === true || block?.matches("minecraft:air")) && !block?.hasTag("km_machine")) {
                    ...
                    if(e.getComponent(EntityCT.Inventory)) { dropInventory(e); }
...
function dropInventory(te : Entity) {
...
function tileEntityRotation(te : Entity, block: Block) {
    if(!block.matches("minecraft:air")) {
        let rot = 0;
...
function getTileEntity(dim : Dimension, coords : Vector3) {
...
function checkForDimensions() {
    let dimensions = [];
    if (w.getDimension("overworld")) {dimensions.push("overworld")}
    if (w.getDimension("nether")) {dimensions.push("nether")}
    if (w.getDimension("the_end")) {dimensions.push("the_end")}
...
function turbineTick(turbine : Entity) {
    let worldNetId = "km:network_"+turbine.getDynamicProperty("km:netId");
    let euNet = (w.getDynamicProperty(worldNetId) as number);
    if (euNet < 32) {
...
function eFurnaceTick(e : Entity) {
    let block = e.dimension.getBlock(e.location);
...
function debugTool(data) {
    ...
    let worldNetId : string = "km:network_"+te.getDynamicProperty("km:netId");
    {
